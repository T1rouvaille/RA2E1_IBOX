...
static void Ibox_timer_expired(struct ktimer *timer_id){
	//printk("timer\n");
...
/*
 * Not super efficient since we use interrupt rx but polling (blocking) Tx code but that shouldn't matter
 * for the application of just pulling service logs and it does simplify the code a lot.  Just dont expect to do anything
 * while we're transmitting as this will block.
 */
...
//    gpio_pin_set_dt(&uarttx, 1);
  k_busy_wait(30);  // load TMR1 value for ~1.5 baud
...
//    gpio_pin_toggle_dt(&uarttx);
    k_busy_wait(25);            // load corrected baud value
  }

  k_busy_wait(26);               // wait for stop bit, ensure serial port is free
//    gpio_pin_set_dt(&uarttx, 1);
...
void Ibox_init(void)
{
    ...
    k_timer_start(&Ibox_timer, K_SECONDS(20), K_NO_WAIT);
}
