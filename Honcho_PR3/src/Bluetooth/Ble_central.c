...
volatile uint8_t remote_data = 0, connected_flag_fortime = 0;
uint32_t var = 0;

struct bt_le_scan_param phy_scan_param = {
    .type = BT_LE_SCAN_TYPE_ACTIVE,
    .interval = BT_GAP_SCAN_FAST_INTERVAL,
    .window = BT_GAP_SCAN_FAST_WINDOW,
    .options = BT_LE_SCAN_OPT_CODED | BT_LE_SCAN_OPT_NO_1M
    };
static struct bt_conn_le_create_param *phy_conn_params = BT_CONN_LE_CREATE_PARAM(
...
K_TIMER_DEFINE(PairBond_timer, PairBond_timer_expired, PairBond_timer_expired);
...
// 	macaddr[8] =  (uint8_t)((NRF_FICR->DEVICEADDR[1] >> 8) | 0xC0); // 2MSB must be set 11			//@KJ
// 	*/
// 	// char addr_s[BT_ADDR_LE_STR_LEN];
// 	bt_addr_le_t addr = {0};
// 	size_t count = 1;
// 	struct bt_le_oob oob1;

// 	bt_le_oob_get_local(BT_ID_DEFAULT,&oob1);
// 	// bt_addr_le_to_str(&oob1.addr, addr_s, sizeof(addr_s));
// 	// if(memcmp(oob1.addr.a.val,0,6))
// 	// {
// 	// 	macaddr[3] = (uint8_t)NRF_FICR->DEVICEADDR[0];
// 	// 	macaddr[4] = (uint8_t)(NRF_FICR->DEVICEADDR[0] >> 8);
// 	// 	macaddr[5] = (uint8_t)(NRF_FICR->DEVICEADDR[0] >> 16);
...
// 	// else 
// 	// {
// 		macaddr[3] = oob1.addr.a.val[5];
// 		macaddr[4] = oob1.addr.a.val[4]; 
// 		macaddr[5] = oob1.addr.a.val[3]; 
// 		macaddr[6] = oob1.addr.a.val[2]; 
// 		macaddr[7] = oob1.addr.a.val[1];
// 		macaddr[8] = oob1.addr.a.val[0];
// 	// }
// }
			...
			if (data->data_len % sizeof(uint8_t) != 0U) {
				...
				if (manuf_data.companyID1 == SBD_ID1 && manuf_data.companyID2 == SBD_ID2  && manuf_data.PT_Index1 == SBD_PT_Index1 ){
					...
					if ((manuf_data.status1.bits.b7 == 1) && (status_flags.request_pair == 0)){
						...
						// err = bt_conn_le_create(addr, BT_LE_CONN_PARAM_DEFAULT, param, &default_conn); 	//@KJ
						err = bt_conn_le_create(addr, phy_conn_params,param, &default_conn);
						if (err) {
							printk("Create conn failed (err %d)\n", err);
							start_scan();				//@KJ
						}
						printk(" Unit bonded new pair request \n");
						DL_Inc_U32(&DL_log.unique_devices_paired_with_unit);
...
static void phy_device_found(struct bt_scan_device_info *device_info,
			      struct bt_scan_filter_match *filter_match,
			      bool connectable)
{
    int err;
    struct bt_filter_status btfs;

    printk("PHY Device found\n");

    // Simply stripping data to use the standard device found function
    const bt_addr_le_t *addr = device_info->recv_info->addr;
    int8_t rssi = device_info->recv_info->rssi;
    uint8_t type = device_info->recv_info->adv_type;
    struct net_buf_simple *ad = device_info->adv_data;

    char addr_str[BT_ADDR_LE_STR_LEN];
    bt_addr_le_to_str(addr, addr_str, sizeof(addr_str));

    err = bt_scan_filter_status_get(&btfs);
    if (err)
    {
        printk("Filter status get failed\n");
    }
...
BT_SCAN_CB_INIT(phy_device_found_cb, phy_device_found, NULL, NULL, NULL);
BT_SCAN_CB_INIT(device_found_cb, device_found, NULL, NULL, NULL);
...
void scan_init(void)
{
    ...
    struct bt_scan_init_param scan_init = {
        .connect_if_match = 0,
        .scan_param = &phy_scan_param,
        .conn_param = NULL};

    bt_scan_init(&scan_init);
    bt_scan_cb_register(&phy_device_found_cb);
...
static void start_scan(void)
{
	...
	uint8_t bt_scan_data_array[2] = {SBD_ID1, SBD_ID2};
    struct bt_scan_manufacturer_data bt_scan_data = {
        .data = bt_scan_data_array,
        .data_len = 2,
    };
    ...
    err = bt_scan_filter_add(BT_SCAN_FILTER_TYPE_MANUFACTURER_DATA, &bt_scan_data);
    if (err)
    {
        printk("Scanning filters MANUF Data cannot be set (err %d)\n", err);
        return;
    }
    err = bt_scan_filter_enable(BT_SCAN_ALL_FILTER , false);
    if (err)
    {
        printk("Filters MANUF Data cannot be turned on (err %d)\n", err);
        return;
    }
    printk("Manufacturer Data Filter Enabled\n");
    
    err = bt_scan_start(BT_SCAN_TYPE_SCAN_ACTIVE);
		// err = bt_le_scan_start(&phy_scan_param, phy_device_found);
			       ...
			       void *context)
{
	...
	// k_timer_stop(&ScanLedBlink_timer);
	k_timer_stop(&PairBond_timer);						//Added by @KJ
	gpio_pin_set_dt(&bleled, 1);
	status_flags.is_connected = 1;
	connected_flag_fortime =1;
	DL_Inc_U32(&DL_log.bluetooth_connection_with_unit);
}
...
//	int err;
	...
	if(status_flags.is_connected == 1)
		DL_Inc_U32(&DL_log.dropped_connections_with_a_paired_device);
	motor_stop();
	status_flags.is_connected = 0;
	connected_flag_fortime =0;
//	if (default_conn != conn) {
//		return;
//	}
...
//**********************************************************************************************************************
static void custom_mac_addr(const char *mac_recd)
{
	int err;
	bt_addr_le_t custom_mac;

	err = bt_addr_le_from_str(mac_recd, "public", &custom_mac);
	if (err) {
		printk("Invalid BT address (err %d)\n", err);
	}

	// if (custom_mac.type == BT_ADDR_LE_PUBLIC) 
	// {
		bt_ctlr_set_public_addr(custom_mac.a.val);
		// printk("rched here \n ");
		return;
...
// printk(" here 02\n");
	...
	// custom_mac_addr("B3:AB:FE:AA:BB:22");				//@KJ
...
{	
	...
	set_tx_power(BT_HCI_VS_LL_HANDLE_TYPE_SCAN, 0, txp[2]);
