...
void DL_Log_App_Time(uint32_t run_time)
{
	...
	{
		DL_Inc_U32(&log.app_time_bucket_0);
	}
	else if(run_time < (LOG_TICKS_PER_MIN * 60))
	{
		DL_Inc_U32(&log.app_time_bucket_1);
	}
	else if(run_time < (LOG_TICKS_PER_MIN * 120))
	{
		DL_Inc_U32(&log.app_time_bucket_2);
	}
	else
	{
		DL_Inc_U32(&log.app_time_bucket_3);
	}
 ...
 void DL_Increment_History( fault_t fault )
{
    for(int i=DL_FAULT_HISTORY_SIZE-1;i>0;i--){
		log.fault_history[i] = log.fault_history[i-1];
	}
	log.fault_history[0]=(uint8_t)fault;
...
void DL_Update_Shutdown_Log(/*pass vars holding shutdown fault flags*/ )
{
	...
	faultmask = log.fault_history[0]+1;
...
int DL_Init_NVM(void){
	...
	fs.offset = FLASH_AREA_OFFSET(storage);//237568
	...
	if(DL_LOG_SIZE != sizeof(log)) // catch log sz errors
	{
		printk("Log structure size error!\n"); 
		return 11;
	}
	rc = nvs_read(&fs, LOG_ID, &log, sizeof(log));
	printk("reading bytes=%d rc=%d\n",sizeof(log),rc); 
	if ((rc > 0) && (log.check_word == DL_CHECK_WORD)) { /* item was found, show it */
		// print out basic logged parameters
		printk("nvs meta data: start Idx: %d, writes: %d, check word: %x\n", LOG_ID, log.Log_Cntr, log.check_word);
		printk("app time 0: %d, 1: %d, 2: %d 3: %d\n", log.app_time_bucket_0,log.app_time_bucket_1,log.app_time_bucket_2,log.app_time_bucket_3 );
		printk("switch presses: %d\n", log.scan_toggle_cycles);
		printk("_________________________________________________________\n");
		printk("\n");
...
void DL_Write(void){
		int err;
		err = nvs_write(&fs, LOG_ID, &log, sizeof(log));
		if( err < 0)
			printk("error writing to flash %d", err);
		else
