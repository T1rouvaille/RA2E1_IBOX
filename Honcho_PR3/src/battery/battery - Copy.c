...
#define VTHERMAL DT_PATH(vthermal)
...
#define THERMAL_ADC_GAIN ADC_GAIN_1_6
#endif
...
//==for thermistor===================================================================
static const struct divider_config th_divider_config = {
#if DT_NODE_HAS_STATUS(VTHERMAL, okay)
	.io_channel = {
		DT_IO_CHANNELS_INPUT(VTHERMAL),
	},
#if DT_NODE_HAS_PROP(VTHERMAL, power_gpios)
	.power_gpios = {
		DT_GPIO_LABEL(VTHERMAL, power_gpios),
		DT_GPIO_PIN(VTHERMAL, power_gpios),
		DT_GPIO_FLAGS(VTHERMAL, power_gpios),
	},
#endif
	.output_ohm = DT_PROP(VTHERMAL, output_ohms),
	.full_ohm = DT_PROP(VTHERMAL, full_ohms),
#else /* /VTHERMAL exists */
	.io_channel = {
		DT_IO_CHANNELS_INPUT(ZEPHYR_USER),
	},
#endif /* /VTHERMAL exists */
};

static struct divider_data th_divider_data = {
#if DT_NODE_HAS_STATUS(VTHERMAL, okay)
	.adc = DEVICE_DT_GET(DT_IO_CHANNELS_CTLR(VTHERMAL)),
#else
	.adc = DEVICE_DT_GET(DT_IO_CHANNELS_CTLR(ZEPHYR_USER)),
#endif
};

static int th_divider_setup(void)
{
	const struct divider_config *cfg = &th_divider_config;
	const struct io_channel_config *iocp = &cfg->io_channel;
	const struct gpio_channel_config *gcp = &cfg->power_gpios;
	struct divider_data *ddp = &th_divider_data;
	struct adc_sequence *asp = &ddp->adc_seq;
	struct adc_channel_cfg *accp = &ddp->adc_cfg;
	int rc;
...
static bool thermal_ok;

static int thermal_setup(const struct device *arg)
{
	int rc = th_divider_setup();

	thermal_ok = (rc == 0);
	LOG_INF("thermal setup: %d %d", rc, thermal_ok);
	return rc;
}

SYS_INIT(thermal_setup, APPLICATION, CONFIG_APPLICATION_INIT_PRIORITY);

int thermal_measure_enable(bool enable)
{
	int rc = -ENOENT;

	if (thermal_ok) {
		const struct divider_data *ddp = &th_divider_data;
		const struct gpio_channel_config *gcp = &th_divider_config.power_gpios;
...
int thermal_sample(void)
{
	int rc = -ENOENT;

	if (thermal_ok) {
		struct divider_data *ddp = &th_divider_data;
		const struct divider_config *dcp = &th_divider_config;
		struct adc_sequence *sp = &ddp->adc_seq;
